...
function App() {
  ...
  */
  ...
  const parseMetadata = (metadata) => {
    ...
    return { name: "Unnamed Agent", description: { expertise: "No description" } };
  };
            ...
            {agentData.map((agent, index) => (
              <li key={index} className="dropdown-item">
                ...
                <div className="routerPrice">
                  {agent.prices && agent.prices.length > 0
                    ? `${ethers.utils.formatEther(agent.prices[0].amount)} ${agent.prices[0].token}`
                    : 'Price not set'}
                </div>
                <div className="routerReputation">
                  {Array(parseInt(agent.metadata?.reputation) || 0).fill().map((_, i) => (
                    <div key={i} className={`point${i + 1}`}></div>
                  ))}
                </div>
                {agent.metadata?.isRouter && <div className="routerBadge">Router</div>}
              </li>
            ))}
          </ul>
        )}
